...
template <typename T> T trilinear(const std::array<T, 8>& c, const std::array<T, 3>& t) noexcept {
    return lerp(
...
template <typename T> T hermite(T p0, T p1, T m0, T m1, T t) noexcept {
    T t2 = t * t;
    T t3 = t2 * t;
...
template <typename T> T catmullRom(T p0, T p1, T p2, T p3, T t) noexcept {
    T t2 = t * t;
    T t3 = t2 * t;
...
/// Quadratic Bezier curve at parameter t.
/// @tparam T Value type.
...
template <typename T> T quadraticBezier(T p0, T p1, T p2, T t) noexcept {
    T s = T(1) - t;
...
/// @param t Curve parameter in [0, 1].
/// @return The interpolated value.
template <typename T> T cubicBezier(T p0, T p1, T p2, T p3, T t) noexcept {
    T s = T(1) - t;
...
template <typename T> T bicubicCatmullRom(const T* tap16, T tx, T ty) noexcept {
    std::array<T, 4> rows;
    for (int r = 0; r < 4; ++r) {
        rows[static_cast<size_t>(r)] =
            catmullRom(tap16[r * 4 + 0], tap16[r * 4 + 1], tap16[r * 4 + 2], tap16[r * 4 + 3], tx);
    }
...
template <typename T> T cubicKernel(T x, T a = T(-0.5)) noexcept {
    x = std::abs(x);
    T x2 = x * x, x3 = x2 * x;
    if (x < 1)
...
template <typename T> T lanczosKernel(T x, int a) noexcept {
    if (x == 0)
        return 1;
    if (std::abs(x) >= a)
        return 0;
